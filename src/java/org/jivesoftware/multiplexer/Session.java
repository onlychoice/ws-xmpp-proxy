...
/**
 * The session represents a connection between the server and a client (c2s) or another server (s2s)
 * as well as a connection with a component. Authentication and user accounts are associated with
 * c2s connections while s2s has an optional authentication association but no single user user.
 * <p>
 * 
...
public abstract class Session {
    ...
    /**
     * The user name used by this session.
     */
     ...
     * processed. Key: stream ID, value: the session.
     */
    ...
    /**
     * Closes connections of connected clients since the server or the connection manager is being
     * shut down. If the server is the one that is being shut down then the connection manager will
     * keep running and will try to establish new connections to the server (on demand).
     */
    ...
    /**
     * Set the new status of this session. Setting a status may trigger certain events to occur
     * (setting a closed status will close this session).
     * 
     * @param status
    ...
    /**
     * Returns a text with the available stream features. Each subclass may return different values
     * depending whether the session has been authenticated or not.
     * 
    ...
    /**
     * Indicate the server that the session has been closed. Do nothing if the session was the one
     * that originated the close action.
     */
