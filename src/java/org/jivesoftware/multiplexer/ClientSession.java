...
public class ClientSession extends Session {
    ...
    public static Session createSession(String serverName, XmlPullParser xpp, Connection connection)
        ...
        if (!xpp.getName().equals("stream") && !isFlashClient) {
            throw new XmlPullParserException(LocaleUtils
                    .getLocalizedString("admin.error.bad-stream"));
        }

        if (!xpp.getNamespace(xpp.getPrefix()).equals(ETHERX_NAMESPACE)
                && !(isFlashClient && xpp.getNamespace(xpp.getPrefix()).equals(FLASH_NAMESPACE))) {
            throw new XmlPullParserException(LocaleUtils
                    .getLocalizedString("admin.error.bad-namespace"));
        }
        ...
        // Set the user name of this session.
        String userName = xpp.getAttributeValue("", "from");
    ...
    /**
     * Closes the client connection. The <tt>systemStopped</tt> parameter indicates if the client
     * connection is being closed because the server is shutting down or unavailable or if it is
     * because the connection manager is being shutdown.
     * 
     * @param systemStopped
     *            true when the server is no longer available or the connection manager is being
     *            shutdown.
     */
