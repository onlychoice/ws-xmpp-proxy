...
public class ConnectionWorkerThread extends Thread {
    ...
    /**
     * JID that identifies this connection to the server. The address is composed by the connection
     * manager name and the name of the thread. e.g.: connManager1/thread1
     */
    ...
    private boolean compressConnection(XMPPPacketReader reader, StringBuilder openingStream)
        ...
        Connection.CompressionPolicy compressionPolicy = Connection.CompressionPolicy
                .valueOf(policyName);
        ...
        if (Connection.CompressionPolicy.optional == compressionPolicy) {
            ...
            if (zlibSupported) {
                ...
                connection
                        .deliverRawText("<compress xmlns='http://jabber.org/protocol/compress'><method>zlib</method></compress>");
                ...
                if ("compressed".equals(answer.getName())) {
                    ...
                    ZInputStream in = new ZInputStream(connection.getTLSStreamHandler()
                            .getInputStream());
                ...
                Log.debug("CM - Stream compression found but zlib method is not supported by"
                        + serverName);
            }
            return false;
        }
    ...
    private boolean doHandshake(String streamID, XMPPPacketReader reader) throws Exception {
        ...
        if (proceed != null && proceed.getName().equals("handshake")) {
            Log
                    .debug("OS - Handshake was SUCCESSFUL with host: " + serverName + " id: "
                            + streamID);
            return true;
        }
    ...
    /**
     * Creates a reader that will process incoming packets from the server. Incoming stanzas will be
     * handled by {@link ServerPacketHandler} through a pool of threads.
     * 
     * @param reader
     *            the reader to use to retrieve stanzas.
     */
    ...
    public void clientSessionCreated(String streamID, InetAddress address) {
        ...
        sb.append("'><session xmlns='http://jabber.org/protocol/connectionmanager' id='").append(
                streamID);
     ...
     * 
     * @param streamID
     *            the stream ID assigned by the connection manager to the closed session.
     */
    ...
    public void run() {
        try {
            ...
            ConnectionManager.getInstance().getServerSurrogate().serverConnections
                    .remove(getName());
            // Close the connection
            connection.close();
        }
